...
本项目刻意维护两个独立模型，训练数据范围不同，用途不可混用：
  ...
  full —— 使用全部 50000 条
           用于生成正式 A1/A2 提交物，也是平台默认的生产推理模型。
           默认 as-of 基准日 = 2026-03-26。

若用 full 模型回放 2026-02 之后的已知标签并计算指标，等于用见过答案的模型自评，
指标会虚高且不可信；平台正式推理可以使用 full，但现场模型效果验证必须使用 demo。
元数据记录 `train_cutoff` 与 `eval_range`，加载时可核对用途。
