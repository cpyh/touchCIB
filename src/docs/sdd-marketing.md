...
起投能力等过滤，再形成 Top3。执行渠道与产品排序解耦：先按当日最新画像
动态生成 Top200 经理池，池外客户再按静态画像规则分流至 App、电话或短信。
...
flowchart TB
    subgraph BATCH["阶段一：全局批次"]
        ...
        S3 --> S4["客户内 Top3"]
        S4 --> S5["最新画像排序<br/>动态生成经理池 Top200"]
    end
...
| 渠道 | `channel_manager_quota` | 硬 | 仅当日动态经理池成员可使用 manager |
...
- 工作台在未联系的池内客户中按最新 A1 机会分取前 12 位作为“今日任务”。完成联系后记录 `sent` 并进入等待回流，下一位自动补入；未处理客户不写状态，次日随新预测重新入池和排序。
- 不设置超时释放或回落状态机；这与每日全量 8000 客户重算机制一致。
